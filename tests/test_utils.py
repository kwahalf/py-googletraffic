...
class TestClassifyTrafficArray(unittest.TestCase):
    ...
    def test_non_traffic_array(self):
        ...
        # Black array (0, 0, 0) - should be far from all traffic colors
        array = np.full((2, 2, 3), [0, 0, 0], dtype=np.uint8)
        result = classify_traffic_array(array)

        # All values should be in valid range 0-4
        self.assertTrue(bool(np.all(result >= 0)))
        self.assertTrue(bool(np.all(result <= 4)))
    ...
    def test_non_traffic_color_blue(self):
        ...
        # Blue should either be 0 or at least be a valid level (0-4)
        self.assertIn(level, [0, 1, 2, 3, 4], f"Classification must return value 0-4, got {level}")
...
class TestCalculateBounds(unittest.TestCase):
    ...
    def test_bounds_at_equator(self):
        ...
        bounds = calculate_bounds(center_lat=0, center_lng=0, width=1000, height=1000, zoom=10)
...
class TestSplitBoundsIntoTiles(unittest.TestCase):
    ...
    def test_small_area_single_tile(self):
        ...
        bounds = {"north": 40.01, "south": 40.00, "east": -73.99, "west": -74.00}
    ...
    def test_tile_coverage(self):
        ...
        bounds = {"north": 40.1, "south": 40.0, "east": -73.9, "west": -74.0}
