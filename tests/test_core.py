...
class TestMakeRaster(unittest.TestCase):
    ...
    def test_make_raster_with_different_dimensions(self, mock_capture):
        ...
        from googletraffic.core import make_raster
        from googletraffic.core import classify_traffic_array
    ...
    def test_multiple_tiles(self, mock_rio, mock_split, mock_make):
        ...
        mock_split.return_value = [
            {'center_lat': 40.75, 'center_lng': -73.98, 
             'width': 500, 'height': 500, 'position': (0, 0)},
            {'center_lat': 40.75, 'center_lng': -73.94, 
             'width': 500, 'height': 500, 'position': (0, 1)},
            {'center_lat': 40.79, 'center_lng': -73.98, 
             'width': 500, 'height': 500, 'position': (1, 0)},
            {'center_lat': 40.79, 'center_lng': -73.94, 
             'width': 500, 'height': 500, 'position': (1, 1)},
        ]
...
class TestMakeRasterFromPolygon(unittest.TestCase):
    ...
    def test_with_geodataframe(self, mock_bbox):
        ...
        try:
            ...
            # Test
            result = make_raster_from_polygon(
                polygon=gdf,
                zoom=14,
                google_key="test_key"
            )
